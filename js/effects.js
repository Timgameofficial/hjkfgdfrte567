...
(function (global) {
  ...
  const manualQuality = MBStorage.get('mb_quality', null);
  if (manualQuality) tier = manualQuality;
  document.body.dataset.quality = tier;

  function setQuality(t, manual = true) {
    tier = t;
    document.body.dataset.quality = t;
    if (manual) MBStorage.set('mb_quality', t);
  }
  ...
  function monitorFrame() {
    ...
    if (frameTimes.length > 60) {
      ...
      if (avg > 33 && tier !== 'low' && !manualQuality) { // sustained < ~30fps, auto-tuning only
  ...
  global.MBEffects = {
    ...
    getParticleBudget, getBlurBudget, getTier, setQuality,
