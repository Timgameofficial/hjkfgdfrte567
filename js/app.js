...
(function () {
  ...
  function startMotionListening() {
    MBShake.start(
      (level) => { clearTimeout(motionWatchdog); runShakeSequence(level); },
      (level, speed) => {
        clearTimeout(motionWatchdog);
        // live feedback while shaking before lock triggers full sequence
        if (!isRevealing) {
          ballWrap.classList.remove('shake-weak', 'shake-medium', 'shake-strong');
          ballWrap.classList.add('shake-' + level);
          MBAudio.playShakeTick(level === 'strong' ? 2 : level === 'medium' ? 1.3 : 0.7);
        }
      }
    );
  ...
  function runShakeSequence(level) {
    if (isRevealing) return;
    isRevealing = true;
    resetIdleTimer();
