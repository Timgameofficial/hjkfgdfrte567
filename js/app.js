...
(function () {
  ...
  function armMotionWatchdog() {
    ...
    motionWatchdog = setTimeout(() => { manualShakeBtn.classList.remove('hidden'); MBBall.setManualModeActive(true); }, 7000);
  }

  function attemptAsk() {
    unlockAudioOnce();
    getStats().sessionStreak; // no-op read to keep intent clear
  ...
  function init() {
    ...
    // handlers must be registered regardless of platform so the desktop
    // "click to predict" fallback (MBShake.forceRelease) has something to call
    MBShake.start({ onTick: handleShakeTick, onRelease: handleShakeRelease, onOvercharge: handleOvercharge });
