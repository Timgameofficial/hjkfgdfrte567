...
(function () {
  ...
  let consecutiveOvercharges = 0; // in-memory only, resets on reload — feeds the "interaction_intensity" personality trigger
  ...
  function getStats() { return MBStorage.getMerged(STATS_KEY, defaultStats()); }
  function saveStats(s) { MBStorage.set(STATS_KEY, s); }

  // "Session streak" is meant to count consecutive questions IN THIS VISIT
  // (that's what the achievements/personality lines describe: "3 questions
  // in a row per visit"). It previously only ever incremented and was never
  // reset anywhere, so it was silently identical to totalQuestions for the
  // lifetime of the app — every "streak" achievement was really just an
  // alias for "total questions ever asked". Resetting it once per page load
  // (a fresh session) restores its intended meaning; maxStreak still tracks
  // the best streak ever achieved across all visits.
  function resetSessionStreak() {
    const s = getStats();
    if (s.sessionStreak !== 0) { s.sessionStreak = 0; saveStats(s); }
  }

  function computeDayStreak(daysUsed) {
    // uses MBDate's LOCAL calendar day, not toISOString() — see dateutil.js
    // for why the UTC-based version rolls the day over at the wrong local
    // time and silently breaks the streak near midnight
    const set = new Set(daysUsed);
    let streak = 0;
    let d = new Date();
    while (true) {
      const key = MBDate.localDayKey(d);
  ...
  // ===================== PERSONALITY (see personality.js) =====================
  // The old version of this only checked totalQuestions counts; it's now
  // folded into MBPersonality.pick(), called once per reveal from
  // finishReveal() with the full shared context object.
  ...
  function runCinematicReveal(level, flags) {
    if (isRevealing) return;
    isRevealing = true;
    setInputLocked(true); // visually disable Ask + input so double-taps during the reveal are obvious no-ops, not silent ignores
  ...
  function finishReveal(answerObj, level, flags) {
    ...
    // ---- shared context, built once, fed to memory/signs/secrets/personality ----
    const memResult = MBMemory.recordQuestion(question, answerObj);
    const stats = recordAnswer(answerObj, MBShake.getIntensity());

    const connBefore = MBProgression.getConnection();
    MBProgression.registerInteraction();
    const connAfter = MBProgression.getConnection();

    if (flags.overcharge) consecutiveOvercharges++; else consecutiveOvercharges = 0;

    const ctx = {
      question, qNorm: memResult.norm, answerObj,
      rarity: answerObj.rarity, category: answerObj.category,
      stats, memory: memResult,
      connection: connAfter, connectionTierChanged: connBefore.label !== connAfter.label,
      xp: MBProgression.getXpProgress(), hour: new Date().getHours(),
      overcharge: !!flags.overcharge, perfect: !!flags.perfect,
      consecutiveOvercharges, pastEntry: MBMemory.randomPastEntry(),
      achievementsUnlockedCount: MBAchievements.unlockedCount()
    };

    const secretEvent = MBSecretEvents.evaluate(ctx);
    ctx.secretEventId = secretEvent ? secretEvent.id : null;
    ctx.secretEvent = secretEvent;
    const newSigns = MBSigns.evaluate(ctx);

    const isUnanswered = !!secretEvent && secretEvent.id === 'unanswered';
    const displayText = isUnanswered ? 'Некоторые вопросы должны остаться без ответа.' : answerObj.text;

    // ---- visual/audio reveal (unchanged from before) ----
    ...
    const revealText = () => {
      ballText.style.opacity = '0';
      setTimeout(() => {
        ballText.textContent = displayText;
        ballText.classList.toggle('legendary-text', !isUnanswered && ['legendary', 'mythic', 'secret'].includes(answerObj.rarity));
        ballText.style.opacity = '1';
        if (!isUnanswered && answerObj.rarity !== 'common') {
    ...
    // ---- personality / signs / secret-event announcements ----
    // staggered so they never all land in the same instant as the reveal
    // itself, or on top of each other
    let announceDelay = 900;
    if (secretEvent) {
      setTimeout(() => applySecretEvent(secretEvent, ctx), announceDelay);
      announceDelay += 900;
    }
    newSigns.forEach(sign => {
      setTimeout(() => announceSign(sign), announceDelay);
      announceDelay += 850;
    });
    const personalityLine = MBPersonality.pick(ctx);
    if (personalityLine) {
      setTimeout(() => MBUI.showToast('🔮 ' + personalityLine.message), announceDelay);
    }

    setHintLines('Задай новый вопрос в уме...', 'Потряси телефон ещё раз 🔮');
    resultActions.classList.remove('hidden');
    resultActions.dataset.answer = displayText;
  ...
  // ---- secret event effects (visual/audio/toast only — the engine itself
  // stays UI-agnostic, see secretevents.js) ----
  function applySecretEvent(event, ctx) {
    switch (event.id) {
      case 'question_100':
        MBUI.showToast('💯 Сотый вопрос. Особенный момент.');
        MBBall.triggerRings(6);
        MBEffects.legendaryScreenFlash();
        MBProgression.addXP(100, (info) => MBUI.showLevelUp(info));
        break;
      case 'void':
        MBUI.showToast('🕳️ Пустота ответила тишиной.');
        MBEffects.dimScreen(0.55, 300);
        setTimeout(() => MBEffects.undimScreen(600), 500);
        MBHaptics.overcharge();
        break;
      case 'unanswered':
        MBUI.showToast('❓ Шар отказался отвечать.');
        break;
      case 'fracture':
        MBUI.showToast('⚡ Трещина в реальности.');
        MBEffects.chromaticPulse(600);
        break;
      case 'paradox':
        MBUI.showToast(`🔀 Раньше я говорил иначе: «${ctx.memory.priorAnswer}»`);
        break;
      case 'echo':
        MBUI.showToast(`🔁 Эхо: «${ctx.memory.priorAnswer}»`);
        break;
      case 'resonance':
        MBUI.showToast('⚡ Резонанс.');
        MBEffects.screenShake(260, 1.5);
        MBBall.setStarSpeed(2.2);
        setTimeout(() => MBBall.setStarSpeed(1), 1800);
        break;
      case 'memory_fragment':
        MBUI.showToast('🧩 Осколок памяти: что-то очень похожее ты уже спрашивал.');
        break;
      case 'lucky_moment':
        MBUI.showToast('🍀 Удачный момент.');
        MBProgression.addXP(10, (info) => MBUI.showLevelUp(info));
        break;
      case 'omen':
        MBUI.showToast('🔮 Знамение записано.');
        break;
    }
  }

  function announceSign(sign) {
    MBUI.showToast(`${sign.icon} Новый знак: ${sign.name}`);
  }
  ...
  // ---- input locking during a reveal (visual feedback for the isRevealing guard above) ----
  function setInputLocked(locked) {
    askBtn.disabled = locked;
    questionInput.disabled = locked;
    askBtn.classList.toggle('is-locked', locked);
  }

  // ---- rotating placeholder so an empty input still feels alive between questions ----
  const PLACEHOLDERS = [
    'Что ты хочешь узнать?', 'О чём подумать шару?', 'Задай любой вопрос...',
    'Стоит ли рискнуть?', 'Спроси о том, что беспокоит...'
  ];
  let placeholderIdx = 0;
  function rotatePlaceholder() {
    if (document.activeElement === questionInput || questionInput.value) return;
    placeholderIdx = (placeholderIdx + 1) % PLACEHOLDERS.length;
    questionInput.placeholder = PLACEHOLDERS[placeholderIdx];
  }
  setInterval(rotatePlaceholder, 6000);

  questionInput.addEventListener('keydown', (e) => {
    if (e.key === 'Enter') { e.preventDefault(); attemptAsk(); }
    else if (e.key === 'Escape') { questionInput.value = ''; }
  });
  ...
  function renderStats() {
    ...
    $('collection-card').innerHTML = `
      ...
      <div style="font-size:12.5px;color:var(--text-1);margin-top:8px;">Знаки: ${MBSigns.discoveredCount()} / ${MBSigns.totalCount()}</div>
      <div class="signs-grid" id="signs-grid"></div>
    `;
    MBSigns.render($('signs-grid'));
  }
  ...
  function init() {
    // a fresh page load is a new "visit" — see resetSessionStreak() above
    resetSessionStreak();
    ...
    if (isMobileLike() && MBShake.isSupported() && !MBShake.needsPermission()) {
      // NOTE: do NOT arm the motion watchdog here. armMotionWatchdog() sets
      // `awaitingShake = true`, which attemptAsk() interprets as "the user
      // already pressed Ask and is waiting for a shake — if they press Ask
      // again, just give them an answer". Arming it this early (before the
      // user has pressed Ask even once) meant the very FIRST tap of
      // "Спросить" always matched that branch and instantly fired
      // MBShake.forceRelease(), completely bypassing real shake detection —
      // this was the root cause of "shaking does nothing". The watchdog is
      // now armed only from within attemptAsk(), once the user has actually
      // asked (see below), which is where it belongs.
