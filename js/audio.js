...
(function (global) {
  ...
  function playReveal(rarity) {
    switch (rarity) {
      case 'mythic':
        tone({ freq: 220, freqEnd: 2200, type: 'sine', duration: 1.6, gain: 0.28 });
        tone({ freq: 660, type: 'triangle', duration: 1.8, gain: 0.2, delay: 0.2 });
        tone({ freq: 880, type: 'sine', duration: 1.2, gain: 0.15, delay: 0.45 });
        noiseBurst({ duration: 1.1, gain: 0.22, filterFreq: 3400 });
        break;
      case 'legendary':
      ...
      case 'rare':
        ...
        break;
      case 'uncommon':
        tone({ freq: 600, freqEnd: 820, type: 'sine', duration: 0.4, gain: 0.19 });
        break;
      default:
  ...
  function playCharge(intensityPct) {
    const p = Math.max(0, Math.min(1, intensityPct / 100));
    tone({ freq: 60 + p * 260, type: 'sawtooth', duration: 0.12, gain: 0.05 + p * 0.12 });
  }

  function playLightning() {
    noiseBurst({ duration: 0.18, gain: 0.22, filterFreq: 4200 });
    tone({ freq: 1600, freqEnd: 300, type: 'square', duration: 0.12, gain: 0.06 });
  }

  function playPerfectShake() {
    tone({ freq: 880, type: 'sine', duration: 0.18, gain: 0.2 });
    tone({ freq: 1320, type: 'sine', duration: 0.22, gain: 0.16, delay: 0.1 });
  }
  ...
  global.MBAudio = {
    ...
    playCharge, playLightning, playPerfectShake
  };
})(window);
