...
(function (global) {
  ...
  // ===================== TAB VISIBILITY =====================
  // Background tabs must not keep animating/painting: it wastes battery and
  // CPU for no visible benefit, and (see quality.js) throttled RAF timing
  // while hidden can also fool the FPS-based quality auto-tuner. Each loop
  // below checks `pageHidden` and simply skips re-scheduling itself while
  // hidden; `resumeLoops()` restarts all three the moment the tab is shown
  // again (each RAF chain is otherwise self-contained, so this is safe to
  // call even if a loop is already running).
  ...
  function init() {
    ...
    // Debounced so a drag-resize or a device rotation doesn't re-seed
    // particles and recompute gradients on every intermediate pixel — only
    // once the size settles. ResizeObserver on the ball wrap itself also
    // catches layout changes that don't fire a window 'resize' event at all
    // (e.g. the on-screen keyboard opening/closing on some mobile browsers).
    let resizeDebounce = null;
    const doResize = () => { resizeBg(); resizeAtmo(); resizeBallCanvas(); };
    const scheduleResize = () => { clearTimeout(resizeDebounce); resizeDebounce = setTimeout(doResize, 120); };
    window.addEventListener('resize', scheduleResize);
    if (typeof ResizeObserver !== 'undefined') {
      new ResizeObserver(scheduleResize).observe(document.getElementById('ball-wrap'));
    }
