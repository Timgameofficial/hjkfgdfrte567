...
(function (global) {
  ...
  // ===================== CHARGE STATE (drives fog speed / glow intensity) =====================
  function setChargeLevel(pct) {
    // pct: 0-100, read by CSS via custom property for fog speed / glow scale
    const wrap = document.getElementById('ball-wrap');
    if (!wrap) return;
    const clamped = Math.max(0, Math.min(100, pct));
    wrap.style.setProperty('--charge', (clamped / 100).toFixed(3));
  }

  function spawnEnergyRing() {
    const wrap = document.getElementById('ball-wrap');
    if (!wrap || MBEffects.prefersReducedMotion) return;
