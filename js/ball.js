...
(function (global) {
  ...
  // ===================== PARALLAX INPUT (background layer only) =====================
  // Ball-surface tilt & highlight movement are owned by physics.js (spring-driven);
  // this just keeps the background nebula/stars drifting with the same input.
  ...
  function enableOrientationParallax() {
    ...
    window.addEventListener('deviceorientation', (e) => {
      ...
      parallax.x = nx * 20;
      parallax.y = ny * 20;
      if (global.MBPhysics) global.MBPhysics.onGyro(nx, ny);
    }, true);
  }

  // ===================== TOUCH / DRAG ON THE BALL ITSELF =====================
  // Drag response + ripple-on-touch (spec §6/§13). Wheel/drag rotation that
  // already existed in ui.js (enableBallDrag) is left untouched; this adds the
  // spring-driven tilt + ripple layer on top.
  function initTouchPhysics(wrapEl) {
    if (!wrapEl || !global.MBPhysics) return;
    ...
    function pos(e) {
      ...
      return {
        nx: clamp(((cx - r.left) / r.width - 0.5) * 2, -1, 1),
        ny: clamp(((cy - r.top) / r.height - 0.5) * 2, -1, 1),
        cx, cy, r
      };
    }
    ...
    function down(e) {
      dragging = true;
      const p = pos(e);
      global.MBPhysics.onTouchDrag(p.nx, p.ny, true);
      spawnRipple(p.cx - p.r.left, p.cy - p.r.top, p.r.width);
    }
    function move(e) {
      if (!dragging) return;
      const p = pos(e);
    ...
    function up() {
      if (!dragging) return;
      dragging = false;
      global.MBPhysics.onTouchDrag(0, 0, false);
      global.MBPhysics.releaseTouch();
    }
  ...
  function spawnRipple(x, y, size) {
    const layer = document.getElementById('ball-ripple-layer');
    if (!layer || MBEffects.prefersReducedMotion) return;
    const r = document.createElement('div');
    r.className = 'touch-ripple';
    r.style.left = x + 'px';
    r.style.top = y + 'px';
    r.style.width = r.style.height = Math.round(size * 0.12) + 'px';
    layer.appendChild(r);
    setTimeout(() => r.remove(), 720);
  }

  // ===================== VORTEX / CORE SPEED CONTROL =====================
  function setVortexSpeed(speedMul) {
    const vortex = document.getElementById('ball-vortex');
    if (vortex) vortex.style.animationDuration = Math.max(0.6, 9 / Math.max(speedMul, 0.01)) + 's';
  }
  function collapseVortex(on) {
    const vortex = document.getElementById('ball-vortex');
    if (vortex) vortex.classList.toggle('collapsing', !!on);
  }
  function setCoreState(state) {
    // state: 'idle' | 'compress' | 'bloom'
    const core = document.getElementById('ball-core');
    if (!core) return;
    core.classList.toggle('core-compress', state === 'compress');
    core.classList.toggle('core-bloom', state === 'bloom');
  }
  ...
  function init() {
    ...
    const wrap = document.getElementById('ball-wrap');
    if (global.MBPhysics) {
      global.MBPhysics.start({
        ball: document.getElementById('ball'),
        highlight: document.getElementById('ball-highlight'),
        highlight2: document.getElementById('ball-highlight-2'),
        wrap
      });
    }
    initTouchPhysics(wrap);
  }
